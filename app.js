...
function renderSuspects() {
  $('#suspectGrid').innerHTML = suspects.map((suspect, index) => `
    <button class="suspect-card" type="button" data-suspect="${suspect.id}" style="--accent:${['#c46650','#497a96','#497b60','#837094'][index]}">
      ...
      <span class="avatar ${suspect.className}" role="img" aria-label="${suspect.name} 角色代號 ${suspect.initials}"><span>${suspect.initials}</span></span>
      <h3>${suspect.name}</h3>
...
function openSuspect(id) {
  ...
  $('#dialogContent').innerHTML = `
    <article class="dialog-profile">
      <div class="avatar ${suspect.className}" role="img" aria-label="${suspect.name} 角色代號 ${suspect.initials}"><span>${suspect.initials}</span></div>
      <h3>${suspect.name}</h3>
...
function renderDossier() {
  ...
  $('#voteList').innerHTML = suspects.map(suspect => `
    <label class="vote-option">
      ...
      <span><span class="avatar ${suspect.className}" aria-hidden="true"><span>${suspect.initials}</span></span><span class="vote-person"><b>${suspect.name}</b><small>${suspect.role}</small></span><span class="vote-check" aria-hidden="true">✓</span></span>
    </label>`).join('');
